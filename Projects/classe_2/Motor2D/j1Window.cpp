...
bool j1Window::Awake(pugi::xml_node node)
{
	...
	else
	{
		...
		bool borderleess = node.child("boderless").attribute("value").as_bool(false);
		...
		width = node.child("resolution").attribute("width").as_int(640);
		height = node.child("resolution").attribute("height").as_int(480);
		scale = node.child("resolution").attribute("height").as_int(1);
